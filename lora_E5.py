...
class LoRaE5:
    ...
    """
    def __init__(self, uart = None, rx_pin = 4, tx_pin = 5, speed = 9600, debug=True):
        """
    ...
    def wait_for_rx_message(self, timeout : int = 5000)-> str:
        ret = ''
        end = time.ticks_ms()+timeout
        while (time.ticks_ms() < end):
            msg = self.read_data(True)
            if len(msg) > 0:
                split_msg = msg.split("\n")
                #print(f"Split Message: {split_msg}")
                for line in split_msg:
                    if "+MSG: PORT" in line:
                        split_msg = line.split(" ")
                        #print(f"Split Line: {split_msg}")
                        return split_msg[len(split_msg)-1].strip("\"\r")   
        return None
